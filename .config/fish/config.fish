...
if status is-interactive
    ...
    set -gx CLAUDE_CODE_EFFORT_LEVEL high # env var: low|medium|high (max is /effort slash-cmd only)
...
───────────────────────────────────────────────────────────────"
    ...
    # System monitoring functions moved to functions/ for lazy loading:
    # killp, psf, psg, port, ports, mem, cpu, netstat-tuln, procmon, portmon,
    # myports, dnslookup, topmon, sysinfo
    ...
    # kctx, kns, kpod, kcluster, kpods-all, kpf, kexec, ktop,
    # dcon, tfw, awsp, secsan, portscan, logsf, benchf, tfcostf
    #
    # Helm FZF integration: helm_fzf_native.fish
