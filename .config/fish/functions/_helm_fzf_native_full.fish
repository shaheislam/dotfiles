...
function __helm_releases_fzf --description "Browse all releases with Alt key actions"
    ...
    # Alt+D: Diff between ANY two revisions - side-by-side nvim diff mode
    # Two-step selection: pick first revision, then second revision
    set -l diff_cmd "bash -c 'exec </dev/tty >/dev/tty 2>&1; ns=\"{1}\"; rel=\"{2}\"; echo \"Select two revisions to compare...\"; echo \"\"; rev1=\$(helm history \"\$rel\" -n \"\$ns\" --output json | jq -r \".[] | \\\"\\(.revision)\\\\t\\(.status)\\\\t\\(.updated)\\\\t\\(.description)\\\"\" | fzf --height=50% --delimiter=\"\\t\" --with-nth=1,2,3,4 --header=\"Select FIRST revision\" --preview=\"helm get manifest \$rel -n \$ns --revision {1} 2>/dev/null | bat --color=always --language=yaml --style=plain | head -50\" --preview-window=right:50%:wrap); if [ -z \"\$rev1\" ]; then exit 0; fi; rev1_num=\$(echo \"\$rev1\" | cut -f1); echo \"First revision: r\$rev1_num\"; echo \"\"; rev2=\$(helm history \"\$rel\" -n \"\$ns\" --output json | jq -r \".[] | select(.revision != \$rev1_num) | \\\"\\(.revision)\\\\t\\(.status)\\\\t\\(.updated)\\\\t\\(.description)\\\"\" | fzf --height=50% --delimiter=\"\\t\" --with-nth=1,2,3,4 --header=\"Select SECOND revision (comparing against r\$rev1_num)\" --preview=\"helm diff revision \$rel \$rev1_num {1} -n \$ns 2>/dev/null | bat --color=always --language=diff --style=plain | head -100\" --preview-window=right:60%:wrap); if [ -z \"\$rev2\" ]; then exit 0; fi; rev2_num=\$(echo \"\$rev2\" | cut -f1); echo \"Comparing r\$rev1_num vs r\$rev2_num\"; file1=/tmp/helm-manifest-\$rel-r\$rev1_num.yaml; file2=/tmp/helm-manifest-\$rel-r\$rev2_num.yaml; helm get manifest \"\$rel\" -n \"\$ns\" --revision \$rev1_num > \"\$file1\" 2>/dev/null; helm get manifest \"\$rel\" -n \"\$ns\" --revision \$rev2_num > \"\$file2\" 2>/dev/null; TERM=\$TERM nvim -d \"\$file1\" \"\$file2\" < /dev/tty > /dev/tty; rm -f \"\$file1\" \"\$file2\"'"
