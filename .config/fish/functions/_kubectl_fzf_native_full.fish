...
function kubectl_fzf_native --description "FZF-powered kubectl completion using native functions"
    ...
    if test "$show_preview" = "true"; and test -n "$preview_cmd"
        ...
        set -l f7_cmd "bash -c 'containers=\$(kubectl get pod {} -o jsonpath=\"{.spec.containers[*].name}\"); count=\$(echo \$containers | wc -w); if [ \$count -eq 1 ]; then target=\$containers; else exec </dev/tty >/dev/tty 2>&1; target=\$(echo \$containers | tr \" \" \"\\n\" | fzf --height=40% --prompt=\"Target container: \"); fi; kubectl debug {} -it --image=ghcr.io/shaheislam/netshoot-nvim:latest --share-processes --target=\$target -- bash'"
