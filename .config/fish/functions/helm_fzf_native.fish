...
function __helm_releases_fzf --description "Browse all releases with Alt key actions"
    ...
    set -l diff_cmd "bash -c 'exec </dev/tty >/dev/tty 2>&1; ns=\"{1}\"; rel=\"{2}\"; curr=\$(helm history \"\$rel\" -n \"\$ns\" -o json | jq -r \".[0].revision\"); echo \"Diff against current revision: r\$curr\"; echo \"\"; rev=\$(helm history \"\$rel\" -n \"\$ns\" --output json | jq -r \".[] | \\\"\\(.revision)\\\\t\\(.status)\\\\t\\(.updated)\\\\t\\(.description)\\\"\" | fzf --height=50% --delimiter=\"\\t\" --with-nth=1,2,3,4 --header=\"Select revision to compare (LEFT=old, RIGHT=current)\" --preview=\"helm diff revision \$rel {1} \$curr -n \$ns 2>/dev/null | bat --color=always --language=diff --style=plain | head -100\" --preview-window=right:60%:wrap); if [ -n \"\$rev\" ]; then rev_num=\$(echo \"\$rev\" | cut -f1); old_file=/tmp/helm-manifest-\$rel-r\$rev_num.yaml; new_file=/tmp/helm-manifest-\$rel-r\$curr.yaml; helm get manifest \"\$rel\" -n \"\$ns\" --revision \$rev_num > \"\$old_file\" 2>/dev/null; helm get manifest \"\$rel\" -n \"\$ns\" --revision \$curr > \"\$new_file\" 2>/dev/null; TERM=\$TERM nvim -d \"\$old_file\" \"\$new_file\" < /dev/tty > /dev/tty; rm -f \"\$old_file\" \"\$new_file\"; fi'"
