...
function __helm_releases_fzf --description "Browse all releases with Alt key actions"
    ...
    # Alt+D: Diff between revisions (requires helm-diff plugin) - interactive revision selection
    set -l diff_cmd "bash -c 'exec </dev/tty >/dev/tty 2>&1; ns=\"{1}\"; rel=\"{2}\"; curr=\$(helm history \"\$rel\" -n \"\$ns\" -o json | jq -r \".[0].revision\"); echo \"Diff against current revision: r\$curr\"; echo \"\"; rev=\$(helm history \"\$rel\" -n \"\$ns\" --output json | jq -r \".[] | \\\"\\(.revision)\\\\t\\(.status)\\\\t\\(.updated)\\\\t\\(.description)\\\"\" | fzf --height=50% --delimiter=\"\\t\" --with-nth=1,2,3,4 --header=\"Select revision to compare\" --preview=\"helm diff revision \$rel {1} \$curr -n \$ns 2>/dev/null | bat --color=always --language=diff --style=plain | head -100\" --preview-window=right:60%:wrap); if [ -n \"\$rev\" ]; then rev_num=\$(echo \"\$rev\" | cut -f1); tmpfile=/tmp/helm-diff-\$rel-r\$rev_num-r\$curr.diff; helm diff revision \"\$rel\" \$rev_num \$curr -n \"\$ns\" > \"\$tmpfile\" 2>/dev/null && nvim -R \"\$tmpfile\" || echo \"helm-diff plugin not installed. Install: helm plugin install https://github.com/databus23/helm-diff\"; rm -f \"\$tmpfile\"; fi'"
