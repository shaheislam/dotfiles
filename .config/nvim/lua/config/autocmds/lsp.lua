...
-- Consolidates all LSP-related autocmds with improvements
...
function M.setup()
  ...
  -- Auto-refresh code lens on specific events (opt-in)
  ...
  -- Toggle inlay hints based on mode
  vim.api.nvim_create_autocmd({ "LspAttach" }, {
    ...
    callback = function(args)
      ...
      if client and client.server_capabilities.inlayHintProvider then
        ...
        -- Optionally toggle based on insert mode
  ...
  vim.api.nvim_create_autocmd("LspProgress", {
    ...
    callback = function(args)
      ...
      if value.kind == "end" then
        vim.notify(
          string.format("%s: %s", client.name, value.title or "Operation completed"),
          vim.log.levels.INFO,
          { title = "LSP Progress", timeout = 2000 }
        )
      end
    end,
  })
