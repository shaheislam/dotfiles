...
- **OpenCode workflows**: [docs/opencode-nvim.md](docs/opencode-nvim.md) covers the OpenCode-first `opencode.nvim` setup, SSE logging into Entire via `.config/opencode/plugin/sse-recorder.ts`, Diffview replay tooling, and the `wrapped.nvim` dashboard for Neovim-config analytics.
