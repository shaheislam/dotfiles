...
### Agent Window Status
Tmux window colors are event-driven. Claude wrapper/hooks and OpenCode `tmux-open.sh` set window-scoped `@wname_style`; OpenCode status is mirrored through `.config/opencode/plugin/tmux-status.ts` state files.
