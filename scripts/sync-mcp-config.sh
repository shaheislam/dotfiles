...
# --- Codex: append [mcp_servers.*] sections to .codex/config.toml ---
codex_config="$REPO_ROOT/.codex/config.toml"

if [ -f "$codex_config" ]; then
    # Check if MCP servers are already configured
    if grep -q '\[mcp_servers\.' "$codex_config" 2>/dev/null; then
        echo "[codex] MCP servers already in config.toml (skipping — edit manually if needed)"
    else
        # Generate TOML [mcp_servers.*] from .mcp.json
        # Codex uses SSE format for remote MCPs and command format for local
        mcp_toml=$(jq -r '.mcpServers | to_entries[] |
            "\n[mcp_servers.\(.key)]\ntype = \"stdio\"\ncommand = \"\(.value.command)\"\nargs = [\(.value.args | map("\"" + . + "\"") | join(", "))]"' "$MCP_FILE" 2>/dev/null || true)

        if [ -n "$mcp_toml" ]; then
            if $DRY_RUN; then
                echo "[codex] Would append to config.toml:"
                echo "$mcp_toml"
            else
                {
                    echo ""
                    echo "# --- MCP Servers (synced from .mcp.json) ---"
                    echo "$mcp_toml"
                } >>"$codex_config"
