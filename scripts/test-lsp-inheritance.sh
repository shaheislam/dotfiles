...
test_global_baseline() {
	...
	# Get version
	local gopls_version
	gopls_version=$(gopls version 2>&1 | head -1 || echo "unknown")
...
EOF
	...
	local nix_lsp_enabled
	nix_lsp_enabled=$(nix develop --command bash -c "echo \"\$NIX_LSP_ENABLED\"" 2>/dev/null)
	...
	local gopls_path
	gopls_path=$(nix develop --command which gopls 2>/dev/null)
	...
	local gopls_version
	gopls_version=$(nix develop --command gopls version 2>&1 | head -1 || echo "unknown")
...
test_path_precedence() {
	...
	local first_path
	first_path=$(nix develop --command bash -c "echo \"\$PATH\" | tr ':' '\n' | head -1" 2>/dev/null)
	...
	local nix_profile_position
	nix_profile_position=$(nix develop --command bash -c "echo \"\$PATH\" | tr ':' '\n' | grep -n '.nix-profile' | head -1 | cut -d: -f1" 2>/dev/null)
...
test_rapid_switching() {
	...
	cd "$PROJECT_A"
	local a_path
	a_path=$(nix develop --command which gopls 2>/dev/null)
	print_info "Project A: $a_path"

	# Project B
	cd "$PROJECT_B"
	local b_path
	b_path=$(nix develop --command which gopls 2>/dev/null)
	print_info "Project B: $b_path"

	# Global
	cd "$GLOBAL_DIR"
	local g_path
	g_path=$(command -v gopls)
	print_info "Global: $g_path"

	# Back to Project A
	cd "$PROJECT_A"
	local a2_path
	a2_path=$(nix develop --command which gopls 2>/dev/null)
...
main() {
	...
	test_global_baseline || ((failed++)) || true
	test_project_a || ((failed++)) || true
	test_project_b || ((failed++)) || true
	test_isolation || ((failed++)) || true
	test_path_precedence || ((failed++)) || true
	test_rapid_switching || ((failed++)) || true
