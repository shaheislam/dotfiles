...
run_doctor() {
    ...
    if [[ $issues -eq 0 ]]; then
        ...
        echo ""
        echo "  Recovery steps:"
        echo "    1. colima start --cpu 2 --memory 4 --disk 20 --runtime docker"
        echo "    2. docker compose -f $COMPOSE_FILE up -d"
        echo "    3. sonarqube doctor   # re-run to verify"
        echo "    4. sonarqube status   # confirm server is UP"
        echo "  Named volumes preserve all data across any restart."
    fi
}
...
*)
    ...
    echo ""
    echo "Compatibility:"
    echo "  Edition:  SonarQube Community Edition (free, open source)"
    echo "  APIs:     REST API (issues, quality gates, project status) - all CE"
    echo "  AI:       --ai/--fix are custom Fish wrappers that pipe CE API"
    echo "            results to Claude CLI or Ollama. NOT a SonarQube feature."
    echo "            No SonarQube AI plugins or higher editions required."
    echo "  Scanner:  sonar-scanner CLI (Homebrew). Sends code to local server."
    echo ""
    echo "Language support (CE built-in analyzers):"
    echo "  JavaScript, TypeScript, Python, Java, Go, PHP, Ruby, Scala,"
    echo "  Kotlin, HTML, CSS, XML."
    echo "  NOT included in CE: C, C++, Rust, Swift, Objective-C, ABAP,"
    echo "  COBOL, PL/SQL, T-SQL, Apex (require plugins or paid editions)."
    echo ""
    echo "Token security:"
    echo "  Storage:  File (~/.config/sonarqube/token, chmod 600)"
    echo "            or Fish env var: read -sP 'Token: ' t; set -Ux SONAR_TOKEN \$t"
    echo "  Scanner:  Token is passed via SONAR_TOKEN env var (not -Dsonar.token"
    echo "            CLI arg) to avoid exposure in ps/process list output."
    echo "  History:  NEVER paste tokens directly in shell commands. Use 'read -s'"
    echo "            to avoid persisting tokens in shell history files."
    echo "  Logging:  sonar-scanner does not log the token value. The SONAR_TOKEN"
    echo "            env var is not printed by this script."
    echo "  Shared:   Do not export SONAR_TOKEN in shared shell profiles."
    echo "            Use per-user Fish universal vars (set -Ux) or file storage."
    echo ""
    echo "Quality gate (CI integration):"
    echo "  Scans use -Dsonar.qualitygate.wait=true by default. sonar-scanner"
    echo "  exits non-zero if the gate fails. Wire this into CI pipelines:"
    echo ""
    echo "    # GitHub Actions example:"
    echo "    - name: SonarQube scan"
    echo "      run: sonarqube scan ."
    echo "      # Exit code propagates: 0 = gate passed, non-zero = gate failed"
    echo ""
    echo "    # Pre-push hook example (in .git/hooks/pre-push):"
    echo "    sonarqube scan . || { echo 'Quality gate failed'; exit 1; }"
    echo ""
    echo "Data persistence:"
    echo "  Scan results, project config, and Elasticsearch indexes are stored"
    echo "  in Docker named volumes (sonarqube_data, sonarqube_logs,"
    echo "  sonarqube_extensions). These survive container restarts and Colima"
    echo "  reboots. Only 'sonarqube uninstall' or 'docker volume rm' deletes them."
    echo ""
    echo "Recovery (Docker/Colima failure):"
    echo "  1. colima start                              # Restart Colima VM"
    echo "  2. docker compose -f <compose-file> up -d    # Restart container"
    echo "  3. sonarqube doctor                          # Verify all 8 checks"
    echo "  4. sonarqube status                          # Confirm server is UP"
    echo "  Named volumes are intact after any restart. No data is lost."
    ;;
esac
