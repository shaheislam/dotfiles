# Debian 12 Bookworm Dockerfile for Dotfiles Testing
# Testing dotfiles on stable Debian base - uses setup.sh for complete installation
FROM debian:12

# Metadata
LABEL maintainer="dotfiles-testing"
LABEL description="Debian 12 Bookworm environment for testing dotfiles installation via setup.sh"
LABEL version="2.0"
...
# Usage examples:
# Build: docker build -f scripts/docker/dockerfiles/debian.Dockerfile -t dotfiles-test:debian .
# Run interactive: docker run -it --rm dotfiles-test:debian
# Run tests: docker run --rm dotfiles-test:debian /home/testuser/dotfiles/scripts/docker/scripts/run-all-tests.sh
