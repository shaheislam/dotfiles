...
# ============================================
# CATEGORY 5: Shell Parity
# ============================================
print_header "Category: Shell Parity"
reset_test_counters

print_subheader "Environment Variables"
run_test "Bash, Zsh, and Fish set EDITOR=nvim" \
    "bash -lc 'echo \$EDITOR' | grep -q 'nvim' && zsh -c 'echo \$EDITOR' | grep -q 'nvim' && fish -c 'echo \$EDITOR' | grep -q 'nvim'"

run_test "Bash, Zsh, and Fish set VISUAL=nvim" \
    "bash -lc 'echo \$VISUAL' | grep -q 'nvim' && zsh -c 'echo \$VISUAL' | grep -q 'nvim' && fish -c 'echo \$VISUAL' | grep -q 'nvim'"

run_test "Bash, Zsh, and Fish set LANG" \
    "bash -lc 'echo \$LANG' | grep -q 'en_US' && zsh -c 'echo \$LANG' | grep -q 'en_US' && fish -c 'echo \$LANG' | grep -q 'en_US'"

run_test "Bash, Zsh, and Fish resolve gwt-ticket" \
    "bash -lc 'command -v gwt-ticket' && zsh -c 'command -v gwt-ticket' && fish -c 'command -q gwt-ticket'"
...
CATEGORY_RESULTS["Shell Parity"]=$(print_test_summary "Shell Parity" | grep -q "PASSED\|passed" && echo "PASS" || echo "FAIL")
...
for category in "Cross-Platform Abstractions" "Fish Function Execution" "Plugin Functionality" "Tool Integrations" "Fish Functions" "MCP Server Runtime" "Shell Parity" "Error Scenarios & Graceful Degradation" "Configuration Files" "Performance"; do
