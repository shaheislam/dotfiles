...
pm_init() {
    LINUX_PM=$(detect_package_manager)
    export LINUX_PM
...
pm_map_package_name() {
    ...
    apt)
        ...
        eza | zoxide | starship | yazi | bottom | btop | procs | dust | duf | ast-grep)
    ...
    yum | dnf)
        ...
        eza | zoxide | starship | yazi | btop | procs | dust | duf | ast-grep)
            echo "BINARY_INSTALL:$generic"
            ;;
        mise | kubie | lazydocker | terraform-docs | terragrunt)
            echo "BINARY_INSTALL:$generic"
    ...
    pacman)
        ...
        fd | bat | eza | zoxide | starship | ripgrep | ast-grep | fzf) echo "$generic" ;;
        ...
        npm | python | python-pip | go | rust) echo "$generic" ;;
