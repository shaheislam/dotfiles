#!/usr/bin/env bash
# macOS Dotfiles Runtime Validation Script
...
# Main validation
print_header "macOS Dotfiles Runtime Validation"
...
if echo "$zsh_path" | grep -q "/opt/homebrew/bin"; then
    print_success "Zsh PATH includes Homebrew"
    ((TESTS_PASSED++)) || true
else
    print_error "Zsh PATH missing Homebrew"
    ((TESTS_FAILED++)) || true
fi
...
fish_path=$(fish -c "printf '%s\n' \$PATH")
if echo "$fish_path" | grep -q "/opt/homebrew/bin"; then
    print_success "Fish PATH includes Homebrew"
    ((TESTS_PASSED++)) || true
else
    print_error "Fish PATH missing Homebrew"
    ((TESTS_FAILED++)) || true
fi
...
if [[ "$zsh_os" == "Darwin" ]]; then
    print_success "Zsh detects macOS correctly"
    ((TESTS_PASSED++)) || true
else
...
if [[ "$fish_os" == "Darwin" ]]; then
    print_success "Fish detects macOS correctly"
    ((TESTS_PASSED++)) || true
else
...
test_string="macos-validation-$(date +%s)"
...
# Test EDITOR in Bash
editor_bash=$(bash -lc 'echo $EDITOR')
if [[ "$editor_bash" == "nvim" ]]; then
    print_success "Bash EDITOR set to nvim"
    ((TESTS_PASSED++)) || true
else
    print_warning "Bash EDITOR: $editor_bash (expected nvim)"
    ((TESTS_WARNED++)) || true
fi

# Test EDITOR in Fish
editor_fish=$(fish -c "printf '%s\n' \$EDITOR")
...
if bash -lc 'command -v gwt-ticket' &>/dev/null && zsh -c 'command -v gwt-ticket' &>/dev/null && fish -c 'command -q gwt-ticket' &>/dev/null; then
    print_success "Bash, Zsh, and Fish resolve gwt-ticket"
    ((TESTS_PASSED++)) || true
else
    print_error "Shell parity command resolution failed for gwt-ticket"
    ((TESTS_FAILED++)) || true
fi
...
# Test Zsh aliases
if zsh -c 'type python' | grep -q "python3"; then
...
# Test Fish aliases
if fish -c 'type ls' | grep -q "eza"; then
...
if [[ $TESTS_FAILED -eq 0 ]]; then
    ...
    echo -e "${GREEN}✓ Your macOS dotfiles are fully functional.${NC}"
    exit 0
