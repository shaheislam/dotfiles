...
SESSION_SYNTHESIZER="$SCRIPT_DIR/session-synthesize.sh"
...
# Reconcile missed session files before generating the weekly rollup so scheduled
# synthesis includes sessions that were missed by normal shutdown hooks.
if [[ -x "$SESSION_SYNTHESIZER" ]]; then
	reconcile_args=(--reconcile)
	if $VERBOSE; then
		reconcile_args+=(--verbose)
	fi
	if $VERBOSE; then
		echo -e "${BLUE}Reconciling missed sessions before synthesis...${NC}"
	fi
	if ! bash "$SESSION_SYNTHESIZER" "${reconcile_args[@]}" >/dev/null 2>&1; then
		echo -e "${YELLOW}Warning: session reconciliation failed; continuing with existing session files.${NC}" >&2
	fi
fi
...
if command -v claude &>/dev/null; then
	...
	_claude_out=$(echo "$_claude_out" | sed '$d')
