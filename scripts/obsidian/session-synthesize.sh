...
RECONCILE=false
FORCE_SESSION_ID=""
SYNTH_REASON="${SYNTH_REASON:-manual}"
FORCE=false
DEDUP_SEC="${OBSIDIAN_SYNC_DEDUP_SEC:-60}"
DEDUP_DIR="${TMPDIR:-/tmp}/obsidian-sync-dedup"
...
# Dedup gate: skip if same cwd was synthesised within DEDUP_SEC.
# Reconcile, dry-run, and --force always bypass. Stamp file is per-cwd hash.
if ! $FORCE && ! $DRY_RUN && ! $RECONCILE; then
    mkdir -p "$DEDUP_DIR" 2>/dev/null || true
    DEDUP_KEY=$(echo -n "$PROJECT_CWD" | md5sum | cut -c1-16)
    DEDUP_STAMP="$DEDUP_DIR/$DEDUP_KEY"
    if [[ -f "$DEDUP_STAMP" ]]; then
        last_sync_epoch=$(stat -f %m "$DEDUP_STAMP" 2>/dev/null || stat -c %Y "$DEDUP_STAMP" 2>/dev/null || echo 0)
        now_epoch=$(date +%s)
        elapsed=$((now_epoch - last_sync_epoch))
        if [[ "$elapsed" -lt "$DEDUP_SEC" ]]; then
            echo -e "${YELLOW}Dedup: last sync ${elapsed}s ago (window ${DEDUP_SEC}s, reason=${SYNTH_REASON}), skipping. Use --force to bypass.${NC}" >&2
            exit 0
        fi
    fi
    touch "$DEDUP_STAMP"
...
1. **YAML Frontmatter** — MUST include these exact fields:
   ...
   - tags: [claude-session, project/$REPO_NAME, trigger/$SYNTH_REASON]
