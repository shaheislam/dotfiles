...
test_setup_syntax() {
    ...
    run_test "firefox-history-tab-discard.py syntax valid" "python3 -m py_compile '$DOTFILES_ROOT/scripts/setup/firefox-history-tab-discard.py'"
...
test_browser() {
    ...
    run_test "Firefox setup validates installed policy parity" "grep -q 'validate_policy_install' '$DOTFILES_ROOT/scripts/setup/firefox-setup.sh'"
    run_test "setup.sh exposes Firefox setup outside fonts phase" "grep -q 'setup_firefox_dotfiles' '$DOTFILES_ROOT/scripts/setup.sh'"
    ...
    run_test "Firefox history tab discard helper exists" "[ -f '$DOTFILES_ROOT/scripts/setup/firefox-history-tab-discard.py' ]"
    run_test "Firefox history tab discard helper self-test passes" "python3 '$DOTFILES_ROOT/scripts/setup/firefox-history-tab-discard.py' --self-test"
...
test_opencode() {
    ...
    run_test "OpenCode compatibility shim delegates to real ocv" "grep -q 'find_real_ocv' '$DOTFILES_ROOT/scripts/bin/opencode' && grep -q 'exec \"\$ocv_bin\" \"\$@\"' '$DOTFILES_ROOT/scripts/bin/opencode'"
    run_test "OpenCode shim disables OpenTUI graphics probe" "grep -q 'OPENTUI_GRAPHICS' '$DOTFILES_ROOT/scripts/bin/opencode'"
    run_test "OpenCode shim wraps tmux TUI launches" "grep -q 'tmux-open.sh' '$DOTFILES_ROOT/scripts/bin/opencode' && grep -q 'should_wrap_in_tmux' '$DOTFILES_ROOT/scripts/bin/opencode'"
    run_test "OpenCode shim preserves run and server commands" "grep -q 'run | serve | --help | -h | help | --version | version' '$DOTFILES_ROOT/scripts/bin/opencode'"
    run_test "OpenCode shim wraps option-led TUI launches" "grep -q -- '-\*)' '$DOTFILES_ROOT/scripts/bin/opencode'"
    run_test "OpenCode shim has no tmux color bypass" "! grep -q 'NO_TMUX_STYLE\|NO_TMUX_COLOU\|NO_TMUX_COLOR\|BYPASS_TMUX' '$DOTFILES_ROOT/scripts/bin/opencode'"
    run_test "OpenCode ocv shim exists" "[ -f '$DOTFILES_ROOT/scripts/bin/ocv' ]"
    run_test "OpenCode ocv shim executable" "[ -x '$DOTFILES_ROOT/scripts/bin/ocv' ]"
    run_test "OpenCode ocv shim syntax valid" "bash -n '$DOTFILES_ROOT/scripts/bin/ocv'"
    run_test "OpenCode ocv shim wraps tmux TUI launches" "grep -q 'tmux-open.sh' '$DOTFILES_ROOT/scripts/bin/ocv' && grep -q 'should_wrap_in_tmux' '$DOTFILES_ROOT/scripts/bin/ocv'"
    run_test "OpenCode ocv shim preserves server commands" "grep -q 'serve | --help | -h | help | --version | version' '$DOTFILES_ROOT/scripts/bin/ocv'"
    run_test "OpenCode ocv shim wraps option-led TUI launches" "grep -q -- '-\*)' '$DOTFILES_ROOT/scripts/bin/ocv'"
    run_test "OpenCode ocv shim has no tmux color bypass" "! grep -q 'NO_TMUX_STYLE\|NO_TMUX_COLOU\|NO_TMUX_COLOR\|BYPASS_TMUX' '$DOTFILES_ROOT/scripts/bin/ocv'"
    ...
    run_test "OpenCode tmux launcher guards ocv shim recursion" "grep -q 'OPENCODE_TMUX_WRAPPER_ACTIVE=1' '$DOTFILES_ROOT/scripts/opencode/tmux-open.sh'"
    run_test "OpenCode tmux launcher owns window color" "grep -q 'set_window_style' '$DOTFILES_ROOT/scripts/opencode/tmux-open.sh' && grep -q 'sync_window_style' '$DOTFILES_ROOT/scripts/opencode/tmux-open.sh' && ! grep -q 'tmux-agent' '$DOTFILES_ROOT/.config/opencode/plugin/harness-compat.ts'"
    ...
    run_test "OpenCode tmux launcher uses pane scraping" "grep -q 'capture-pane' '$DOTFILES_ROOT/scripts/opencode/tmux-open.sh' && grep -q 'esc interrupt' '$DOTFILES_ROOT/scripts/opencode/tmux-open.sh'"
    ...
    run_test "OpenCode tmux launcher polls status color" "grep -q 'set_window_style' '$DOTFILES_ROOT/scripts/opencode/tmux-open.sh' && grep -q 'sync_window_style' '$DOTFILES_ROOT/scripts/opencode/tmux-open.sh' && grep -q 'OPENCODE_TMUX_STATUS_INTERVAL' '$DOTFILES_ROOT/scripts/opencode/tmux-open.sh'"
    ...
    run_test "OpenCode skill stats script exists" "[ -f '$DOTFILES_ROOT/scripts/opencode/skill-stats.py' ]"
    run_test "OpenCode skill stats script executable" "[ -x '$DOTFILES_ROOT/scripts/opencode/skill-stats.py' ]"
    run_test "OpenCode skill stats compiles" "python3 -m py_compile '$DOTFILES_ROOT/scripts/opencode/skill-stats.py'"
    run_test "OpenCode skill stats self-test passes" "python3 '$DOTFILES_ROOT/scripts/opencode/skill-stats.py' --self-test"
    run_test "Skill invocation hook exists" "[ -x '$DOTFILES_ROOT/.claude/hooks/log-skill-invocation.py' ]"
    run_test "Skill invocation hook compiles" "python3 -m py_compile '$DOTFILES_ROOT/.claude/hooks/log-skill-invocation.py'"
    run_test "Skill invocation hook self-test passes" "python3 '$DOTFILES_ROOT/.claude/hooks/log-skill-invocation.py' --self-test"
    run_test "Claude tracks skill invocations" "grep -q 'log-skill-invocation.py --harness claude' '$DOTFILES_ROOT/.claude/settings.json'"
    run_test "Claude tracks skill tool invocations" "jq -e '.hooks.PreToolUse[] | select(.matcher == \"Skill\") | .hooks[].command | contains(\"log-skill-invocation.py --harness claude\")' '$DOTFILES_ROOT/.claude/settings.json' >/dev/null"
    run_test "OpenCode tracks skill invocations" "grep -q 'log-skill-invocation.py' '$DOTFILES_ROOT/.config/opencode/plugin/harness-compat.ts' && grep -q 'SKILL_INVOCATION_HARNESS: \"opencode\"' '$DOTFILES_ROOT/.config/opencode/plugin/harness-compat.ts'"
    run_test "OpenCode tracks skill tool invocations" "grep -q 'tool === \"skill\"' '$DOTFILES_ROOT/.config/opencode/plugin/harness-compat.ts' && grep -q 'log-skill-invocation.py' '$DOTFILES_ROOT/.config/opencode/plugin/harness-compat.ts'"
    run_test "OpenCode skill TOIL audit reports inventory" "grep -q 'Skill Inventory' '$DOTFILES_ROOT/scripts/opencode/skill-toil-audit.py' && grep -q 'skill_inventory' '$DOTFILES_ROOT/scripts/opencode/skill-toil-audit.py'"
    run_test "OpenCode monthly skill TOIL audit wrapper exists" "[ -f '$DOTFILES_ROOT/scripts/opencode/skill-toil-audit-monthly.sh' ]"
    run_test "OpenCode monthly skill TOIL audit wrapper executable" "[ -x '$DOTFILES_ROOT/scripts/opencode/skill-toil-audit-monthly.sh' ]"
    run_test "OpenCode monthly skill TOIL audit wrapper syntax valid" "bash -n '$DOTFILES_ROOT/scripts/opencode/skill-toil-audit-monthly.sh'"
    run_test "OpenCode monthly skill TOIL audit rebuilds stats" "grep -q 'skill-stats.py' '$DOTFILES_ROOT/scripts/opencode/skill-toil-audit-monthly.sh' && grep -q 'rebuild-index --days 90' '$DOTFILES_ROOT/scripts/opencode/skill-toil-audit-monthly.sh'"
    run_test "OpenCode monthly skill TOIL audit uses 30 day bounded default" "grep -q -- '--days 30 --min-count 3 --limit 20' '$DOTFILES_ROOT/scripts/opencode/skill-toil-audit-monthly.sh'"
    run_test "OpenCode monthly skill TOIL audit uses per-device local marker" "grep -q 'last-run-month' '$DOTFILES_ROOT/scripts/opencode/skill-toil-audit-monthly.sh' && grep -q 'hostname' '$DOTFILES_ROOT/scripts/opencode/skill-toil-audit-monthly.sh'"
    run_test "OpenCode monthly skill TOIL audit can spawn tmux window" "grep -q 'tmux new-window' '$DOTFILES_ROOT/scripts/opencode/skill-toil-audit-monthly.sh' && grep -q 'SKILL_TOIL_AUDIT_IN_TMUX' '$DOTFILES_ROOT/scripts/opencode/skill-toil-audit-monthly.sh'"
    run_test "OpenCode monthly skill TOIL audit avoids duplicate tmux window" "grep -q 'list-windows' '$DOTFILES_ROOT/scripts/opencode/skill-toil-audit-monthly.sh' && grep -q 'already exists' '$DOTFILES_ROOT/scripts/opencode/skill-toil-audit-monthly.sh'"
    run_test "OpenCode monthly skill TOIL audit tmux command is shell-stable" "grep -q '/bin/bash -lc' '$DOTFILES_ROOT/scripts/opencode/skill-toil-audit-monthly.sh'"
    run_test "OpenCode skill TOIL audit LaunchAgent exists" "[ -f '$DOTFILES_ROOT/Library/LaunchAgents/com.dotfiles.skill-toil-audit.plist' ]"
    run_test "OpenCode skill TOIL audit LaunchAgent is valid" "plutil -lint '$DOTFILES_ROOT/Library/LaunchAgents/com.dotfiles.skill-toil-audit.plist' >/dev/null"
    run_test "OpenCode skill TOIL audit LaunchAgent is monthly" "grep -q '<string>com.dotfiles.skill-toil-audit</string>' '$DOTFILES_ROOT/Library/LaunchAgents/com.dotfiles.skill-toil-audit.plist' && grep -q '<key>StartCalendarInterval</key>' '$DOTFILES_ROOT/Library/LaunchAgents/com.dotfiles.skill-toil-audit.plist' && grep -q '<key>Day</key>' '$DOTFILES_ROOT/Library/LaunchAgents/com.dotfiles.skill-toil-audit.plist'"
    run_test "OpenCode setup installs skill TOIL scheduler" "grep -q '^setup_skill_toil_audit_scheduler()' '$DOTFILES_ROOT/scripts/setup.sh' && grep -q 'com.dotfiles.skill-toil-audit' '$DOTFILES_ROOT/scripts/setup.sh' && grep -q '^[[:space:]]*setup_skill_toil_audit_scheduler$' '$DOTFILES_ROOT/scripts/setup.sh'"
    ...
    run_test "OpenCode forkgwtt command exists" "[ -f '$DOTFILES_ROOT/.opencode/command/forkgwtt.md' ]"
    run_test "OpenCode forkgwtt delegates to Fish wrapper" "grep -q 'opencode-forkworktree' '$DOTFILES_ROOT/.opencode/command/forkgwtt.md'"
    run_test "OpenCode forkgwtt uses build agent" "grep -q '^agent: build$' '$DOTFILES_ROOT/.opencode/command/forkgwtt.md'"
    run_test "OpenCode forkgwtt inherits active model" "! grep -q '^model:' '$DOTFILES_ROOT/.opencode/command/forkgwtt.md'"
    run_test "OpenCode forkgwtt documents plugin intercept" "grep -q 'run without a model call' '$DOTFILES_ROOT/.opencode/command/forkgwtt.md'"
    run_test "OpenCode gwtfork command file removed" "[ ! -f '$DOTFILES_ROOT/.opencode/command/gwtfork.md' ]"
    run_test "OpenCode forkpane command exists" "[ -f '$DOTFILES_ROOT/.opencode/command/forkpane.md' ]"
    run_test "OpenCode forkpane delegates to Fish wrapper" "grep -q 'opencode-forkpane' '$DOTFILES_ROOT/.opencode/command/forkpane.md'"
    run_test "OpenCode forkpane uses build agent" "grep -q '^agent: build$' '$DOTFILES_ROOT/.opencode/command/forkpane.md'"
    run_test "OpenCode forkpane inherits active model" "! grep -q '^model:' '$DOTFILES_ROOT/.opencode/command/forkpane.md'"
    run_test "OpenCode forkpane documents plugin intercept" "grep -q 'run without a model call' '$DOTFILES_ROOT/.opencode/command/forkpane.md'"
    ...
    run_test "OpenCode fork command plugin handles forkgwtt" "grep -q 'FORKGWTT_COMMAND' '$DOTFILES_ROOT/.config/opencode/plugin/fork-command.ts'"
    run_test "OpenCode fork command plugin keeps gwtfork alias" "grep -q 'LEGACY_GWT_FORK_COMMAND' '$DOTFILES_ROOT/.config/opencode/plugin/fork-command.ts'"
    run_test "OpenCode fork command plugin handles forkpane" "grep -q 'FORKPANE_COMMAND' '$DOTFILES_ROOT/.config/opencode/plugin/fork-command.ts'"
    run_test "OpenCode forkgwtt plugin runs wrapper directly" "grep -q 'opencode-forkworktree --session' '$DOTFILES_ROOT/.config/opencode/plugin/fork-command.ts'"
    run_test "OpenCode forkpane plugin runs wrapper directly" "grep -q 'opencode-forkpane --session' '$DOTFILES_ROOT/.config/opencode/plugin/fork-command.ts'"
    run_test "OpenCode forkgwtt plugin uses TUI toasts" "grep -q 'showToast' '$DOTFILES_ROOT/.config/opencode/plugin/fork-command.ts'"
    ...
    run_test "OpenCode tmux idle status is yellow" "grep -q '#\[fg=#e0af68\]' '$DOTFILES_ROOT/scripts/opencode/tmux-open.sh' && grep -q '#\[fg=#e0af68\]' '$DOTFILES_ROOT/scripts/tmux/hooks/tmux-agent-stop.sh'"
    ...
    run_test "OpenCode forkworktree attaches selected fork" "grep -q -- '--opencode-session' '$DOTFILES_ROOT/.config/fish/functions/opencode-forkworktree.fish'"
    run_test "OpenCode forkworktree uses timeline picker" "grep -q 'opencode-select-fork-message' '$DOTFILES_ROOT/.config/fish/functions/opencode-forkworktree.fish'"
    run_test "OpenCode forkworktree forks via API" "grep -q 'opencode-fork-session-api' '$DOTFILES_ROOT/.config/fish/functions/opencode-forkworktree.fish'"
    ...
    run_test "OpenCode forkpane fish wrapper exists" "[ -f '$DOTFILES_ROOT/.config/fish/functions/opencode-forkpane.fish' ]"
    run_test "OpenCode forkpane fish wrapper syntax valid" "fish -n '$DOTFILES_ROOT/.config/fish/functions/opencode-forkpane.fish'"
    run_test "OpenCode forkpane uses attach registry" "grep -q 'opencode/attaches' '$DOTFILES_ROOT/.config/fish/functions/opencode-forkpane.fish'"
    run_test "OpenCode forkpane uses timeline picker" "grep -q 'opencode-select-fork-message' '$DOTFILES_ROOT/.config/fish/functions/opencode-forkpane.fish'"
    run_test "OpenCode forkpane forks via API" "grep -q 'opencode-fork-session-api' '$DOTFILES_ROOT/.config/fish/functions/opencode-forkpane.fish'"
    run_test "OpenCode forkpane uses dynamic split heuristic" "grep -q 'pane_width' '$DOTFILES_ROOT/.config/fish/functions/opencode-forkpane.fish' && grep -q 'pane_height' '$DOTFILES_ROOT/.config/fish/functions/opencode-forkpane.fish'"
    run_test "OpenCode fork message picker exists" "[ -f '$DOTFILES_ROOT/.config/fish/functions/opencode-select-fork-message.fish' ]"
    run_test "OpenCode fork message picker syntax valid" "fish -n '$DOTFILES_ROOT/.config/fish/functions/opencode-select-fork-message.fish'"
    run_test "OpenCode fork message picker uses tmux popup" "grep -q 'display-popup' '$DOTFILES_ROOT/.config/fish/functions/opencode-select-fork-message.fish'"
    run_test "OpenCode fork session API helper exists" "[ -f '$DOTFILES_ROOT/.config/fish/functions/opencode-fork-session-api.fish' ]"
    run_test "OpenCode fork session API helper syntax valid" "fish -n '$DOTFILES_ROOT/.config/fish/functions/opencode-fork-session-api.fish'"
    run_test "OpenCode fork session API helper posts messageID" "grep -q 'messageID' '$DOTFILES_ROOT/.config/fish/functions/opencode-fork-session-api.fish'"
    ...
    run_test "gwt-ticket supports OpenCode session attach" "grep -q -- '--opencode-session' '$DOTFILES_ROOT/.config/fish/functions/gwt-ticket.fish'"
    ...
    run_test "gwt-ticket writes forkgwtt handoff" "grep -q 'forkgwtt.local.md' '$DOTFILES_ROOT/.config/fish/functions/gwt-ticket.fish'"
