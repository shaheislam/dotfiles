...
test_opencode() {
    ...
    run_test "OpenCode setup has dedicated shared server helper" "grep -q '^setup_opencode_shared_server()' '$DOTFILES_ROOT/scripts/setup.sh' && grep -q '^    setup_opencode_shared_server$' '$DOTFILES_ROOT/scripts/setup.sh'"
    ...
    run_test "OpenCode tmux launcher reads window status" "grep -q 'show-window-option.*@opencode_status' '$DOTFILES_ROOT/scripts/opencode/tmux-open.sh' && ! grep -q 'show-environment -g OPENCODE_STATUS' '$DOTFILES_ROOT/scripts/opencode/tmux-open.sh'"
    ...
    run_test "OpenCode Claude compat keeps tmux color on server disposal" "grep -q 'transient OpenCode server' '$DOTFILES_ROOT/.config/opencode/plugin/claude-compat.ts' && grep -q 'wrapper cleanup handles real TUI exit' '$DOTFILES_ROOT/.config/opencode/plugin/claude-compat.ts'"
    ...
    run_test "OpenCode tmux status plugin exists" "[ -f '$DOTFILES_ROOT/.config/opencode/plugin/tmux-status.ts' ]"
    run_test "OpenCode tmux status exports session env" "grep -q 'OPENCODE_SESSION_ID' '$DOTFILES_ROOT/.config/opencode/plugin/tmux-status.ts'"
    ...
    run_test "OpenCode tmux status avoids global coupling" "grep -q 'key !== \"OPENCODE_STATUS\"' '$DOTFILES_ROOT/.config/opencode/plugin/tmux-status.ts' && ! grep -q 'setTmuxEnv(\"OPENCODE_STATUS\"' '$DOTFILES_ROOT/.config/opencode/plugin/tmux-status.ts'"
    ...
    run_test "OpenCode Claude compat plugin configured" "jq -e '.plugin[] | select(. == \"./plugin/claude-compat.ts\")' '$DOTFILES_ROOT/.config/opencode/opencode.json' >/dev/null 2>&1"
    run_test "OpenCode tmux status plugin configured" "jq -e '.plugin[] | select(. == \"./plugin/tmux-status.ts\")' '$DOTFILES_ROOT/.config/opencode/opencode.json' >/dev/null 2>&1"
    run_test "OpenCode Claude subscription plugin configured" "[ -f '$DOTFILES_ROOT/.config/opencode/plugin/opencode-with-claude.ts' ]"
