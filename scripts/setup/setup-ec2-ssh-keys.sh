...
get_ssm_instances() {
    local aws_flags=$(get_aws_flags)

    # Get all running EC2 instances (suppress the info message here)
    local all_instances=$(aws ec2 describe-instances \
...
add_key_to_instance() {
    ...
    local platform="$3"
    local aws_flags=$(get_aws_flags)
    local ssh_key=$(cat "${SSH_KEY_PATH}.pub")
    ...
    for user in "${users[@]}"; do
        ...
        if [ -n "${result}" ] && [ "${result}" != "{}" ]; then
            local command_id=$(echo "${result}" | jq -r '.Command.CommandId // ""')
...
update_ssh_config() {
    ...
    # Generate config entries
    {
        ...
        while IFS='|' read -r id name platform; do
            if [[ "${platform,,}" != windows* ]]; then
                local user=$(get_instance_user "${platform}")
                echo "# ${name}"
                echo "Host ec2-${id}"
                echo "    HostName ${id}"
                echo "    User ${user}"
...
main() {
    ...
    local instances=$(get_ssm_instances)
