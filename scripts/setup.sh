...
ensure_macos_command_line_tools_current() {
    ...
    if ! command_exists softwareupdate; then
        log_verbose "softwareupdate unavailable; skipping Command Line Tools check"
        return 0
    fi

    if ! xcode-select -p >/dev/null 2>&1; then
        if [[ "$DRY_RUN" == "true" ]]; then
            print_warning "DRY RUN: Would require Xcode Command Line Tools installation via 'xcode-select --install'"
            return 0
        fi
    ...
    print_step "Checking macOS Command Line Tools..."

    local clt_label=""
    clt_label=$(softwareupdate --list 2>/dev/null | awk -F': ' '/^\* Label: Command Line Tools for Xcode / {print $2; exit}')

    if [[ -z "$clt_label" ]]; then
        print_success "Command Line Tools already current"
        return 0
    fi
    ...
    print_step "Installing $clt_label..."
    if sudo softwareupdate --install "$clt_label"; then
        print_success "Installed $clt_label"
        return 0
    fi

    print_error "Failed to install $clt_label"
    print_warning "Update Command Line Tools from System Settings or rerun: sudo softwareupdate --install \"$clt_label\""
    return 1
}
...
preflight_checks() {
    ...
    if ! ensure_macos_command_line_tools_current; then
        print_error "Preflight failed: macOS developer tools are not ready"
        exit 1
    fi
...
PTEOF
    ...
    if [[ "$ENABLE_CLAUDE_HEAVY_SETUP" == "true" ]] && command_exists claude; then
        ...
        print_success "Claude Code plugins installed (18 plugins, 9 marketplaces)"
        ...
        # Self-heals plain-file drift: if ~/.claude/settings.json is a regular file
        # (not a symlink), back it up and replace with a symlink to the dotfiles source.
        # This guarantees future edits to the dotfiles copy propagate to every device.
        if [[ -f "$DOTFILES_ROOT/.claude/settings.json" ]]; then
            local target="$HOME/.claude/settings.json"
            local resolved
            resolved=$(readlink "$target" 2>/dev/null || true)
            if [[ ! -L "$target" ]] || [[ "$resolved" != "$DOTFILES_ROOT/.claude/settings.json" ]]; then
                print_step "Linking Claude Code settings from dotfiles..."
                if [[ -f "$target" && ! -L "$target" ]]; then
                    cp -p "$target" "$target.bak.$(date +%s)" 2>/dev/null || true
                fi
...
phase_8_dotfiles() {
    ...
    if command_exists git; then
        ...
        # Git config entries use ~/dotfiles (tilde) paths so git expands them
        # at runtime. This survives home directory or username changes — unlike
        # $HOME or absolute paths which get baked in at setup time.
        # Note: merge drivers and textconv execute shell commands during git
        # diff/merge — acceptable for this trusted personal repo.
        # SC2088: Intentional — literal ~ stored in git config, expanded by git/shell at runtime.
        # This survives home directory or username changes unlike $HOME which bakes in the path.
        ...
        git config --global merge.union-doc.driver "~/dotfiles/scripts/merge-driver-union.sh %A %O %B %L %P"
        ...
        # shellcheck disable=SC2088
        git config --global merge.brewfile.driver "~/dotfiles/scripts/merge-driver-brewfile.sh %A %O %B %L %P"
        ...
        # shellcheck disable=SC2088
        git config --global merge.json-merge.driver "~/dotfiles/scripts/merge-driver-json.sh %A %O %B %L %P"
        ...
        # shellcheck disable=SC2088
        git config --global merge.lockfile.driver "~/dotfiles/scripts/merge-driver-lockfile.sh %A %O %B %L %P"
        log_verbose "Lockfile merge driver registered"

        # Register JSON diff driver - sorts keys for cleaner diffs
        # shellcheck disable=SC2088
        git config --global diff.json.textconv "~/dotfiles/scripts/git-diff-json.sh"
        ...
        # shellcheck disable=SC2088
        git config --global filter.json-normalize.clean "~/dotfiles/scripts/git-clean-json.sh"
    ...
    if [[ -x "$DOTFILES_ROOT/scripts/tools/setup-git-local-excludes.sh" ]]; then
        ...
        for repo in "$HOME/neovim" "$DOTFILES_ROOT"; do
            if [[ -d "$repo/.git" ]]; then
                # Ensure personal repos use github.com-personal SSH alias
                local current_url
                current_url=$(git -C "$repo" remote get-url origin 2>/dev/null || true)
                if [[ "$current_url" == git@github.com:shaheislam/* ]]; then
                    local new_url="${current_url/git@github.com:/git@github.com-personal:}"
                    git -C "$repo" remote set-url origin "$new_url"
                    log_verbose "Fixed remote URL for $repo: $new_url"
                fi
...
phase_9_fonts_and_apps() {
    ...
    if [[ "$DETECTED_OS" == "macos" ]]; then
        ...
        if command_exists aerospace; then
            ...
            local aero_sock="/tmp/bobko.aerospace-$(id -un).sock"
            if [[ -S "$aero_sock" ]]; then
...
" 2>/dev/null || echo "")
        ...
        if [[ -d "$HOME/obsidian" ]] && [[ -f "$DOTFILES_ROOT/scripts/obsidian/install-weekly-synthesis.sh" ]]; then
            print_step "Setting up weekly Claude synthesis scheduler..."
            if bash "$DOTFILES_ROOT/scripts/obsidian/install-weekly-synthesis.sh" >/dev/null 2>&1; then
                print_success "Weekly Claude synthesis scheduler installed"
            else
                log_verbose "Weekly Claude synthesis scheduler setup skipped"
            fi
        fi
...
EOF
            ...
            # Symlink nix.custom.conf from dotfiles (Determinate's sanctioned extension point)
            if [[ "$DETECTED_OS" == "macos" ]]; then
...
MACHINE_CONF
            ...
            # Warn if /etc/nix/nix.conf has settings the current Nix binary doesn't recognise
            local nix_bin="/nix/var/nix/profiles/default/bin/nix"
            if [[ -x "$nix_bin" ]]; then
                local stale_count
                stale_count=$("$nix_bin" show-config 2>&1 | grep -c "unknown setting" || true)
                if [[ "$stale_count" -gt 0 ]]; then
                    print_warning "Nix has $stale_count unknown setting(s) in /etc/nix/nix.conf — update Determinate Nix to fix:"
                    print_warning "  curl --proto '=https' --tlsv1.2 -sSf -L https://install.determinate.systems/nix | sh -s -- upgrade"
                fi
            fi
