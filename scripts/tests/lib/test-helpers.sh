...
test_clipboard() {
    local test_string="validation-test-$$-$(date +%s)"
...
count_fish_plugins() {
    if check_file "$HOME/.config/fish/fish_plugins"; then
        grep -v '^#' "$HOME/.config/fish/fish_plugins" | grep -v '^$' | wc -l | tr -d ' '
    else
        echo "0"
    fi
}
...
count_tmux_plugins() {
    if check_file "$HOME/.tmux.conf"; then
        grep "set -g @plugin" "$HOME/.tmux.conf" | wc -l | tr -d ' '
    else
        echo "0"
    fi
}
...
print_test_summary() {
    ...
    local pass_pct=$(calc_percentage $TESTS_PASSED $TOTAL_TESTS)
    local fail_pct=$(calc_percentage $TESTS_FAILED $TOTAL_TESTS)
    local warn_pct=$(calc_percentage $TESTS_WARNED $TOTAL_TESTS)
    local skip_pct=$(calc_percentage $TESTS_SKIPPED $TOTAL_TESTS)
...
export -f print_success print_error print_warning print_skip print_info
