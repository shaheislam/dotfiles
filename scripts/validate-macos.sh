#!/usr/bin/env bash
# macOS Dotfiles Validation Script
# Tests that all cross-platform changes work correctly on macOS
...
# Main validation
print_header "macOS Dotfiles Validation"

# Test 1: OS Detection
print_header "1. OS Detection"
if [[ "$(uname -s)" == "Darwin" ]]; then
    print_success "macOS detected correctly"
    ((TESTS_PASSED++))
else
    print_error "OS detection failed - not running on macOS"
    ((TESTS_FAILED++))
    exit 1
fi
...
print_header "4. Critical macOS Paths"
run_test_with_warning "Homebrew bin directory exists" "test -d /opt/homebrew/bin"
...
if echo "validation-test" | fish -c "source $HOME/.config/fish/functions/clipboard_copy.fish; clipboard_copy" 2>/dev/null; then
    if pbpaste 2>/dev/null | grep -q "validation-test"; then
        print_success "Clipboard function works correctly"
        ((TESTS_PASSED++))
    else
        print_warning "Clipboard function executed but content not in clipboard"
        ((TESTS_WARNED++))
    fi
else
...
if grep -q 'if \[\[ "$(uname -s)" == "Darwin" \]\]' "$HOME/.zshrc"; then
...
# Test 8: Setup Script Dry-Run
print_header "8. Setup Script Dry-Run (Minimal Profile)"
...
if ./scripts/setup.sh --dry-run --profile minimal &>/tmp/setup-dryrun.log; then
    ...
    if grep -q "Found.*package" /tmp/setup-dryrun.log; then
        local package_count=$(grep "Found.*package" /tmp/setup-dryrun.log | awk '{sum += $2} END {print sum}')
...
# Test 9: Key Tool Availability
print_header "9. Key Tool Availability (Optional)"
run_test_with_warning "Homebrew installed" "command -v brew"
...
print_header "10. Git Repository Status"
cd "$HOME/dotfiles"
...
if [[ $TESTS_FAILED -eq 0 ]]; then
    ...
    echo -e "${GREEN}✓ macOS installation is working correctly.${NC}"
    exit 0
