...
from lib.config import (
    CLAUDE_PROJECTS_DIR, SESSIONS_DIR, MEMORIES_DIR,
    TRIGGER_PATTERNS, FAILURE_PATTERNS, SUCCESS_PATTERNS,
    MAX_MEMORIES_PER_SESSION, MAX_TRANSCRIPT_LENGTH, MEMORY_TYPES
)
    ...
    if triggers:
        ...
        for i, t in enumerate(triggers[:10]):
            trigger_lines.append(
                f'{i+1}. **{t["type"]}** ({t["priority"]}) - "{t["match"]}"'
            )
        triggers_section = f'''
...
'''
    ...
    type_docs = '\n'.join([
        f'- **{t}** ({info["priority"]}): {info["description"]}'
        for t, info in MEMORY_TYPES.items()
    ])
...
def main():
    ...
    if existing_session:
        ...
        created = datetime.fromisoformat(session['created_at'].replace('Z', '+00:00')) if session['created_at'] else datetime.now()
