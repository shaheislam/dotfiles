...
OPENCODE_PREFLIGHT_RUNNER="${CROSS_PROVIDER_OPENCODE_PREFLIGHT:-$HOME/dotfiles/scripts/opencode/check-and-rotate.sh}"

# Codex runner: prefer autorotation wrapper if available, fall back to plain codex CLI.
CODEX_RUNNER_CANDIDATE="${CROSS_PROVIDER_CODEX_RUNNER:-$HOME/dotfiles/scripts/codex/run-with-rotation.sh}"
if [ -n "$CODEX_RUNNER_CANDIDATE" ] && [ -x "$CODEX_RUNNER_CANDIDATE" ]; then
	DEFAULT_CODEX_RUNNER="$CODEX_RUNNER_CANDIDATE"
else
	DEFAULT_CODEX_RUNNER="codex"
fi
...
provider_codex() {
	...
	if [ -n "$profiles_str" ]; then
		...
		for profile in "${profiles[@]}"; do
			...
			local codex_cmd=("$DEFAULT_CODEX_RUNNER" exec)
...
provider_opencode() {
	...
	if [ -x "$OPENCODE_PREFLIGHT_RUNNER" ]; then
		if ! "$OPENCODE_PREFLIGHT_RUNNER" --quiet >/dev/null 2>&1; then
			log_verbose "Provider opencode: usage check failed"
			return 1
		fi
	fi
...
for _p in "${providers[@]}"; do
	...
	if [ "$_p" = "claude" ] && [ -n "${CROSS_PROVIDER_CLAUDE_PROFILES:-}" ] && all_claude_profiles_cooled; then
		log_verbose "Pre-filter: skipping $_p (all Claude profiles cooled)"
