...
Monthly per-device automation is handled by `scripts/opencode/skill-toil-audit-monthly.sh` and `Library/LaunchAgents/com.dotfiles.skill-toil-audit.plist`. The wrapper rebuilds the skill usage SQLite index with `scripts/opencode/skill-stats.py rebuild-index --days 90`, runs the audit with `--days 30 --min-count 3 --limit 20`, prints top 30-day skills and unused 90-day skills, records a local guard in `~/.local/state/opencode/skill-toil-audit/last-run-month`, saves reports by hostname under `~/obsidian/Claude/Audit/skill-toil/<hostname>/` when available, and opens a `skill-toil-YYYY-MM` tmux window only if a tmux server is already running.

Skill invocation tracking is handled by `.claude/hooks/log-skill-invocation.py`. Claude wires it through `UserPromptSubmit`; OpenCode wires it through `.config/opencode/plugin/harness-compat.ts`. The hook stores only privacy-safe metadata in `~/.local/state/agent-skills/invocations.jsonl`: timestamp, harness, skill name, source (`slash` or `tool`), session id, cwd, transcript pointers when available, and prompt hash. `skill-stats.py` derives `~/.local/state/agent-skills/skill-stats.sqlite` from that JSONL plus read-only OpenCode user-message history. The DB stores `invocations`, `skill_summary`, `skill_clusters`, `skill_near_miss`, and conservative `skill_outcomes` rows without raw prompt text. `skill-toil-audit.py` combines that tracker with historical slash prompts to classify skills as active, stale-review, candidate-merge, keep-safety-rare, keep-compatibility-wrapper, or needing a better trigger description.
