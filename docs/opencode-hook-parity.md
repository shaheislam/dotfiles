...
| PreToolUse Edit/Write/MultiEdit guard | `.config/opencode/plugin/claude-compat.ts` -> `settings-edit-redirect.py`, `protect-files.py` | Full | Prevents direct writes to Claude settings, secrets, npm lockfiles, git internals, and generated dependency files. |
...
| PostToolUse Edit/Write/MultiEdit helpers | `.config/opencode/plugin/claude-compat.ts` -> `auto-format.py`, `file-modified.sh`, `ci-lint-on-save.sh`, `plan-watch.sh` | Full | Reuses Claude post-edit helpers for all OpenCode write-like tools. |
| File created or edited opens in Neovim | `.config/opencode/plugin/nvim-open.ts` -> `scripts/nvim-open-file.sh` | Full | Runs only inside tmux, skips noisy/generated paths, and avoids interrupting Neovim insert mode. Covered by `scripts/opencode/test-nvim-open.sh`. |
...
| Cross-provider stop bridge | No OpenCode equivalent yet | Gap | Claude stop payload is more specific than current OpenCode shutdown events. |
