...
## What to Look For
1. **Unnecessary comments**: Comments that restate what the code does
...
5. **Dead code**: Unused imports, unreachable branches, TODO comments without beads tickets
...
8. **AI attribution leakage**: References to "Claude", "AI", "generated" in code or comments
...
5. Commit with format: `refactor: remove unnecessary <what>` (no emojis, no AI references)
...
- For Fish functions: prefer simple `test` conditionals over complex chains
- For Bash scripts: use `;` not `&&` for independent commands per project convention
